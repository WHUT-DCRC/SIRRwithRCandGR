...
def model_load(args):
    ...
    if args.pretrained:
        ...
        if args.model_type == 0:
            ...
            # 这段是不带注意力的分段式网络
            # gen_ckpt_data = torch.load(
            #     r'output/checkpoint_gen_restruct_guidance_100.pth',map_location='cpu')
            # dis_ckpt_data = torch.load(
            #     r'output/checkpoint_dis_restruct_guidance_100.pth')

            # 这段是带注意力的一段式网络
            gen_ckpt_data = torch.load(
                r'output/checkpoint_gen_restruct_guidance_20.pth', map_location='cpu')
            dis_ckpt_data = torch.load(
                r'output/checkpoint_dis_restruct_guidance_20.pth')
